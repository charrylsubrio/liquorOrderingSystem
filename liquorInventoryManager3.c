...
void add_liquor() {
    ...
    fflush(stdin); // Clear input buffer before reading the name
    fgets(liquor_data[num_liquor].name, MAX_NAME - 1, stdin);
...
void save_data() {
    FILE* fp = fopen("liquor.dat", "wb"); // Open file in text write mode with a new filename
    ...
    // Write liquor data in a human-readable format
    fprintf(fp, "Liquor List:\n");
    fprintf(fp, "Name\t\tQuantity\tPrice\n");
    fprintf(fp, "-------\t\t--------\t------\n");
    for (int i = 0; i < num_liquor; i++) {
        fprintf(fp, "%-20s%d\t\t%.2f\n", liquor_data[i].name, liquor_data[i].quantity, liquor_data[i].price);
    }
    ...
    fclose(fp);
    printf("Liquor data saved to 'liquor_inventory.txt' (human-readable format).\n");
}
