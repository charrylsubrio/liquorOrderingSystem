...
int find_user_index(char *username) {
  for (int i = 0; i < num_users; i++) {
    if (strcmp(users[i].username, username) == 0) { // Use strcmp for case-sensitive comparison
      return i;
    }
  }
  return -1;
...
void login() {
  ...
  if (user_index != -1) {
    // Successful username entry, now check password
...
void display_liquors() {
  printf("\nLiquor List:\n\n");
  printf("  LIQUOR NAME\t\t\t\tPRICE\t\tSTOCK\n");
  printf("  -----------\t\t\t\t-------\t\t-----\n");
...
void order_liquor() {
    ...
    // Check if the liquor is found and if it's in stock
    if (liquor_index == -1 || liquors[liquor_index].stock <= 0) {
        printf("Sorry, the liquor is not available or out of stock.\n");
        return;
    }

    printf("Enter the quantity you want to order (available stock: %d): ", liquors[liquor_index].stock);
    scanf("%d", &quantity);

    // Check if the quantity requested exceeds the available stock
    if (quantity > liquors[liquor_index].stock) {
        printf("Sorry, only %d %s are available.\n", liquors[liquor_index].stock, liquors[liquor_index].name);
        return;
    }
