...
void load_data() {
  ...
  while (fgets(liquors[num_liquors].name, MAX_LIQUOR_NAME + 1, liquor_file) != NULL) {
    ...
    // Read price and stock separately
    fscanf(liquor_file, "%d", &liquors[num_liquors].price);
    // Optional: Consume any leftover characters in format buffer
    fgetc(liquor_file);
    fscanf(liquor_file, "%d", &liquors[num_liquors].stock);
